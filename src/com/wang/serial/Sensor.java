					...
					public void dataAvailable() {
						...
						try {
							if (port == null) {
								...
								String raw = ByteUtils
										.byteArrayToHexString(data);
								if (raw.length() == 40) {
									for (int i = 0; i < sensorNum; i++) {
										String address = raw.substring(
												10 + i * 20, 14 + i * 20);// +20
										// System.out.println(address);
										String isWorking = raw.substring(
												16 + i * 20, 18 + i * 20);
										// System.out.println(isWorking);
										if (isWorking.equals("01")
												&& sensorMap.get(address) == false) {
											System.out.println(sensorIndex
													.get(address));
											detectedSensor.add(address);
											sensorMap.put(address, true);
											sensorIndex.put(address,
													detectedSensor
															.indexOf(address));
										} else if (isWorking.equals("02")
												&& sensorMap.get(address) == true) {
											detectedSensor.remove(sensorIndex
													.get(address));
